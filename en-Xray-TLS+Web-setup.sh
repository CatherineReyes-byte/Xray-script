...
check_script_update()
{
    [ "$(md5sum "${BASH_SOURCE[0]}" | awk '{print $1}')" == "$(md5sum <(wget -O - "https://github.com/CatherineReyes-byte/Xray-script/raw/main/Xray-TLS+Web-setup.sh") | awk '{print $1}')" ] && return 1 || return 0
}
#更新脚本
update_script()
{
    if wget -O "${BASH_SOURCE[0]}" "https://github.com/CatherineReyes-byte/Xray-script/raw/main/Xray-TLS+Web-setup.sh" || wget -O "${BASH_SOURCE[0]}" "https://github.com/CatherineReyes-byte/Xray-script/raw/main/Xray-TLS+Web-setup.sh"; then
    ...
    if [ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]; then
        if ! $apt_no_install_recommends -y install "$@"; then
            ...
            if ! $apt_no_install_recommends -y install "$@"; then
                ...
                green  "Welcome to Bug report (https://github.com/CatherineReyes-byte/Xray-script/issues), thank you for your support"
...
swap_off()
{
    if [ $using_swap_now -eq 1 ]; then
        ...
        if swapoff ${temp_dir}/swap && rm -rf ${temp_dir}/swap; then
            ...
            green  "欢迎进行Bug report(https://github.com/CatherineReyes-byte/Xray-script/issues)，感谢您的
支持"
            yellow "Press Enter to continue or Ctrl+c to exit"
            read -s
        fi
        ...
        if (( 1<=choice&&choice<=4 )); then
            if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! dpkg-deb --help | grep -qw "zstd"; then
                ...
                if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]); then
                    ...
                    if ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                        ...
                        if ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                            if ! $apt update; then
                                red "$apt update出错"
                                green  "欢迎进行Bug report(https://github.com/CatherineReyes-byte/Xray-script/issues)，thank you for your support"
                                yellow "Press Enter to continue or Ctrl+c to exit"
                                read -s
                            fi
                            install_dependence linux-base
                ...
                if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                    ...
                    if [ $choice -eq 1 ]; then
                        temp_kernel_sh_url="https://github.com/CatherineReyes-byte/update-kernel/raw/master/update-kernel-stable.sh"
                    elif [ $choice -eq 4 ]; then
                        temp_kernel_sh_url="https://github.com/CatherineReyes-byte/update-kernel/raw/master/update-kernel.sh"
                    else
                        temp_kernel_sh_url="https://github.com/CatherineReyes-byte/xanmod-install/raw/main/xanmod-install.sh"
                    fi
...
readProtocolConfig()
{
    ...
    blue   "   2. gRPC and WebSocket support CDN. For the difference between the two, see: https://github.com/CatherineReyes-byte/Xray-script#About grpc and websocket"
...
readPretend()
{
    ...
    do
        ...
        echo
        yellow " For detailed instructions on choosing a camouflaged website, see：https://github.com/CatherineReyes-byte/Xray-script#伪装网站说明"
        echo
        pretend=""
...
print_config_info()
{
    ...
    echo
    yellow " About fingerprint and alpn，See details：https://github.com/CatherineReyes-byte/Xray-script#About tls handshake tls fingerprint and alpn"
    echo
...
start_menu()
{
    ...
    echo
    tyblue "       official website：https://github.com/CatherineReyes-byte/Xray-script"
    echo
    tyblue "----------------------------------Precautions----------------------------------"
    ...
    tyblue "      https://github.com/CatherineReyes-byte/Xray-script#Installation duration instructions"
